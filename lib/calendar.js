...
var Calendar = _react2.default.createClass({
  ...
  isRangeEnd: function isRangeEnd(date, week) {
    ...
    return range && (range.end.startOf('day').isSame(datetime) || range.start.startOf('day').isSame(datetime));
  },
  inRange: function inRange(date, week) {
    var datetime = this.getDateTime(date, week);
    var range = this.props.range;

    return range && range.start.startOf('day').isBefore(datetime) && range.end.startOf('day').isAfter(datetime);
  },
